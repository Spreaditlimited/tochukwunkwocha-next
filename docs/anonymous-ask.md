...
Share `/ask/question` to invite anonymous questions through a lightweight standalone page (no public navigation, footer, or question feed). The dashboard's Visitor questions tab includes **Copy question link**. The page reuses the compact anonymous form and existing protected submission API, requires no login, and performs no database reads to render. Submissions arrive as visitor questions with public visibility off; only the admin's visibility action exposes them publicly. On success the form is replaced by a question-specific private-delivery confirmation.
