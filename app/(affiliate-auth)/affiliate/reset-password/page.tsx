...
export const metadata: Metadata = buildMetadata({
  ...
  description: "Reset the password for your affiliate partner account.",
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-muted/20 p-5 sm:p-6 lg:p-8">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="mb-8 text-center">
          <Link href="/affiliate/login" className="group mb-6 inline-flex items-center text-sm font-bold text-muted-foreground transition-colors hover:text-primary">
          ...
          <p className="mt-3 text-base text-muted-foreground">
            {firstUse
              ? "Your temporary password has been accepted. Create your private affiliate password to continue."
              : token
                ? "Enter a new password for your affiliate account."
                : "Enter the email used for your affiliate registration."}
          </p>
        </div>

        <PasswordResetForm token={token} accountContext="affiliate" successPath="/dashboard/affiliate" />
      </div>
    </main>
  )
}
