...
function validType(value: unknown): BlogAutomationJobType | null {
  return value === "image" || value === "leadMagnet" || value === "leadMagnetLayout" ? value : null
}
...
export async function POST(request: Request, { params }: { params: Promise<{ pidBlog: string }> }) {
  ...
  if (!type) return NextResponse.json({ error: "Choose image, leadMagnet, or leadMagnetLayout." }, { status: 400 })
  try {
