...
assert.match(liveSessions, /import \{ sendEmail \} from "@\/lib\/email"/)
assert.match(liveSessions, /TEMPORARY_HOSTINGER_SMTP_SESSION_UUID = "live_ptp_batch4_day5"/)
assert.match(liveSessions, /stage === "early_access" \|\| stage === "access_open"/)
assert.match(liveSessions, /shouldUseTemporaryHostingerSmtp\(input\.session\.sessionUuid, input\.stage\)[\s\S]*return sendEmail/)
assert.match(liveSessions, /return sendBrevoTransactionalEmail\(\{[\s\S]*tags: \["live-class-reminder", input\.stage\]/)
