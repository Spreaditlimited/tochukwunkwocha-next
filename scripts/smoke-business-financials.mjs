...
const manualPaymentReview = read("lib/payments/manual-payment-review.ts")
...
assert.match(financials, /WHERE m\.status IN \('approved', 'refunded'\)/)
assert.match(financials, /WHERE o\.payment_status = 'paid'/)
assert.match(financials, /if \(period === "all"\)[\s\S]*from = ""[\s\S]*to = ""/)
assert.match(financials, /export async function recordManualPaymentFinancialTransaction/)
assert.match(financials, /WHERE BINARY m\.payment_uuid = BINARY \$\{paymentUuid\}/)
assert.match(manualPaymentReview, /recordManualPaymentFinancialTransaction\(paymentUuid\)/)
