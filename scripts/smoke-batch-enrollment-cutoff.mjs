...
const installmentsPanel = read("components/student-dashboard/InstallmentsPanel.tsx")
const conversionPanel = read("components/student-dashboard/IndividualToGroupConversionPanel.tsx")
const manualPaymentsPage = read("app/(internal)/internal/(admin)/manual-payments/page.tsx")
const manualPaymentForm = read("app/(internal)/internal/(admin)/manual-payments/AddExternalStudentForm.tsx")
const moduleBatchRules = read("app/(internal)/internal/(admin)/video-library/ModuleBatchRulesClient.tsx")
...
assert.match(utils, /export function formatBatchPickerLabel/)
...
assert.match(dashboard, /const batches = await listCheckoutBatches\(course\.courseSlug\)/)
assert.match(dashboard, /batch\.status\.toLowerCase\(\) === "open"/)
...
assert.match(checkoutForm, /Starts \$\{formatBatchStart\(batch\.batchStartAt\)\}/)
assert.match(installmentsPanel, /formatBatchPickerLabel\(batch\.batchLabel, batch\.batchStartAt\)/)
assert.match(conversionPanel, /formatBatchPickerLabel\(item\.batchLabel, item\.batchStartAt\)/)
assert.match(manualPaymentsPage, /formatBatchPickerLabel\(batch\.batchLabel, batch\.batchStartAt\)/)
assert.match(manualPaymentForm, /formatBatchPickerLabel\(batch\.batchLabel, batch\.batchStartAt\)/)
assert.match(moduleBatchRules, /formatBatchPickerLabel\(batch\.batchLabel \|\| batch\.batchKey, batch\.batchStartAt\)/)
