...
assert.match(utils, /watWallDateTimeMs\(batchStartAt\)/)
assert.match(utils, /currentTimeMs < startTimeMs/)
...
assert.match(batchSwitch, /batch_start_at > DATE_ADD\(UTC_TIMESTAMP\(\), INTERVAL 1 HOUR\)/)
...
assert.match(checkoutForm, /formatBatchRange\(batch\.batchStartAt, batch\.batchEndAt\)/)
