...
type Row={country:string;currency:string;paymentProvider:string;id:string;partnerId:string;hostname:string;years:number;amountMinor:bigint;quoteExpiresAt:Date;status:string;paymentReference:string|null;checkoutUrl:string|null;registrantCiphertext:string|null;registrarOrderId:string|null;expiresAt:Date|null};
const profile=z.object({company:z.string().min(2).max(120),fullName:z.string().min(3).max(180),email:z.string().email().max(190),address1:z.string().min(5).max(240),city:z.string().min(2).max(120),state:z.string().min(2).max(120),postalCode:z.string().min(3).max(40),phone:z.string().regex(/^\+[1-9]\d{7,14}$/)}).strict();
export const platformCommand=z.discriminatedUnion('action',[
 z.object({action:z.literal('quote'),country:z.enum(['NG','GB']).default('NG'),partnerId:z.string().min(1).max(191),hostname:z.string().max(191),years:z.number().int().min(1).max(5)}).strict(),
 z.object({action:z.literal('checkout'),country:z.enum(['NG','GB']).default('NG'),partnerId:z.string().min(1).max(191),quoteId:z.string().max(80),acceptedTotalMinor:z.number().int().positive(),registrant:profile,confirmed:z.literal(true)}).strict(),
...
function result(row:Row){return {orderId:row.id,quoteId:row.id,hostname:row.hostname,amountMinor:Number(row.amountMinor),currency:row.currency||'NGN',status:row.status,checkoutUrl:row.checkoutUrl,expiresAt:row.expiresAt?.toISOString()||null,quoteExpiresAt:row.quoteExpiresAt.toISOString()};}
...
export async function platformDomainCommand(input:z.infer<typeof platformCommand>){
 ...
 if(input.action==='quote'){
  ...
  const quote=await buildPlatformDomainQuote(hostname,input.years,input.country);
  ...
  if(existing)await prisma.$executeRaw`UPDATE domain_platform_orders SET country=${input.country},currency=${quote.currency},paymentProvider=${quote.provider},pricingJson=${JSON.stringify(quote)},amountMinor=${amount}, years=${input.years}, quoteExpiresAt=${expiry}, updatedAt=NOW(3) WHERE id=${id} AND status='QUOTED'`;
  else await prisma.$executeRaw`INSERT INTO domain_platform_orders(id,partnerId,hostname,years,amountMinor,quoteExpiresAt,country,currency,paymentProvider,pricingJson) VALUES(${id},${input.partnerId},${hostname},${input.years},${amount},${expiry},${input.country},${quote.currency},${quote.provider},${JSON.stringify(quote)})`;
 ...
 if(row.paymentProvider!=='stripe'&&!(process.env.PAYSTACK_SECRET_KEY||'').startsWith('sk_live_'))throw new Error('Live domain checkout is not configured.');
 ...
 if(!/^\+(?:44\d{9,10}|234\d{10})$/.test(input.registrant.phone))throw new Error('Enter a valid UK (+44) or Nigerian (+234) phone number.');
 ...
 const payment=row.paymentProvider==='stripe'?await initializeStripe({email:input.registrant.email,amountMinor:Number(row.amountMinor),currency:row.currency,courseName:row.hostname+' domain registration',orderUuid:row.id,courseSlug:'sureimports-domain',idempotencyKey:row.id,metadata:{payment_scope:'sureimports_domain',platformOrderId:row.id},successUrl:siteBaseUrl()+'/api/platform/domains/return',cancelUrl:'https://partner.sureimports.com/partners/dashboard#storefront'}):await initializePaystack({email:input.registrant.email,amountMinor:Number(row.amountMinor),currency:'NGN',reference,metadata:{payment_scope:'sureimports_domain',platformOrderId:row.id},callbackUrl:`${siteBaseUrl()}/api/platform/domains/return`});
 await prisma.$executeRaw`UPDATE domain_platform_orders SET paymentReference=${payment.providerReference||reference},checkoutUrl=${payment.checkoutUrl},updatedAt=NOW(3) WHERE id=${row.id}`;
