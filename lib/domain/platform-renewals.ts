...
export const renewalCommand=z.discriminatedUnion('action',[
 z.object({action:z.literal('renew-quote'),partnerId:z.string().min(1).max(191),orderId:z.string().max(80),years:z.number().int().min(1).max(5)}).strict(),
 z.object({action:z.literal('renew-checkout'),partnerId:z.string().min(1).max(191),renewalId:z.string().max(80),acceptedTotalMinor:z.number().int().positive(),email:z.string().email().max(191),confirmed:z.literal(true)}).strict(),
...
type Row={id:string;partnerId:string;domainOrderId:string;hostname:string;registrarOrderId:string;expectedExpiry:Date;confirmedExpiry:Date|null;years:number;amountMinor:bigint;quoteExpiresAt:Date;status:string;paymentReference:string|null;checkoutUrl:string|null};
function dto(r:Row){return {renewalId:r.id,orderId:r.domainOrderId,hostname:r.hostname,years:r.years,amountMinor:Number(r.amountMinor),currency:'NGN',status:r.status,checkoutUrl:r.checkoutUrl,expiresAt:r.confirmedExpiry?.toISOString()||null};}
...
export async function platformRenewalCommand(input:z.infer<typeof renewalCommand>){
 ...
 if(input.action==='renew-quote'){
  ...
  const price=await registrar.getRegistrationPrice({domainName:domain.hostname,years:input.years,operation:'renew'});
  const quote=await buildDomainQuote(domain.hostname,input.years,'NG',price),id=existing?.id||`SPR_${randomUUID()}`,until=new Date(Date.now()+900000);
  if(existing)await prisma.$executeRaw`UPDATE domain_platform_renewals SET amountMinor=${quote.totalAmountMinor},years=${input.years},quoteExpiresAt=${until},updatedAt=NOW(3) WHERE id=${id} AND status='QUOTED'`;
  else await prisma.$executeRaw`INSERT INTO domain_platform_renewals(id,partnerId,domainOrderId,hostname,registrarOrderId,expectedExpiry,years,amountMinor,quoteExpiresAt) VALUES(${id},${input.partnerId},${domain.id},${domain.hostname},${current.orderId},${expiry},${input.years},${quote.totalAmountMinor},${until})`;
 ...
 if(!(process.env.PAYSTACK_SECRET_KEY||'').startsWith('sk_live_'))throw new Error('Live renewal checkout is unavailable.');
 ...
 const checkout=await initializePaystack({email:input.email,amountMinor:Number(row.amountMinor),currency:'NGN',reference:ref,metadata:{payment_scope:'sureimports_domain_renewal',platformRenewalId:row.id},callbackUrl:`${siteBaseUrl()}/api/platform/domains/return`});
 await prisma.$executeRaw`UPDATE domain_platform_renewals SET checkoutUrl=${checkout.checkoutUrl},updatedAt=NOW(3) WHERE id=${row.id}`;
...
export async function completePlatformRenewal(id:string,partnerId?:string,legacyCheckoutUuid?:string){
 ...
 if(payment.domain!=='live'||payment.reference!==row.paymentReference||payment.amountMinor!==Number(row.amountMinor)||payment.currency!=='NGN'||!metadataMatches)throw new Error('Renewal payment requires reconciliation.');
