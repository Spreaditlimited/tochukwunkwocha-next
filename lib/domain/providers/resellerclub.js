...
function registrantProfile(input) {
  ...
  return {
    ...
    email,
    company: clean(input && input.registrantCompany, 120) || clean(process.env.RESCLUB_CONTACT_COMPANY || process.env.RESELLERCLUB_CONTACT_COMPANY, 120) || name.fullName,
    address1,
    city,
    state,
    country,
...
async function registerDomain(input) {
  ...
  const ids = await resolveRegistrationIds({
    registrantCompany: input && input.registrantCompany,
  ...
  const json = await callApi(
    "/api/domains/register.json",
    {
      ...
      "invoice-option": input && input.externallyPaid === true ? "NoInvoice" : firstEnv(["RESCLUB_INVOICE_OPTION", "RESELLERCLUB_INVOICE_OPTION"], 60) || "KeepInvoice",
...
async function getRegistrationPrice(input) {
  ...
  const findRenewal = (node, depth = 0) => {
    if (!node || typeof node !== 'object' || depth > 5) return null;
    if (node.renewdomain && typeof node.renewdomain === 'object') return node.renewdomain;
    for (const child of Object.values(node)) { const found = findRenewal(child, depth + 1); if (found) return found; }
    return null;
  };
  const addNewPricing = input.operation === 'renew' ? findRenewal(product) || {} : extractAddNewPricingBlock(product) || product.add_new_domain || {};
...
async function getRegistration(input) {
  const domainName = normalizeDomain(input.domainName);
  const details = await callApi('/api/domains/details-by-name.json', { 'domain-name': domainName, options: ['OrderDetails', 'StatusDetails', 'RegistrantContactDetails'] }, 'GET');
  const expiry = Number(details.endtime);
  const orderId = parseDomainOrderId(details);
  if (!looksLikeOrderId(orderId) || !Number.isFinite(expiry) || expiry <= 0 || String(details.domainname).toLowerCase() !== domainName) throw new Error('Registrar registration details are not yet confirmed.');
  return { domainName, orderId, active: String(details.currentstatus).toLowerCase() === 'active', expiresAt: new Date(expiry * 1000).toISOString(), registrantEmail: String(details.registrantcontact?.emailaddr || '').toLowerCase() };
}

// Add individual records; never replace a zone or change its nameservers.
async function addPlatformDnsRecord(input) {
  const domainName = normalizeDomain(input.domainName);
  const endpoints = { TXT: 'add-txt-record', A: 'add-ipv4-record', CNAME: 'add-cname-record' };
  const endpoint = endpoints[input.type];
  if (!endpoint || !domainName) throw new Error('Unsupported DNS record.');
  const host = String(input.host || '').toLowerCase();
  if (host && !/^[a-z0-9_-]+(?:\.[a-z0-9_-]+)*$/.test(host)) throw new Error('Invalid DNS host.');
  const value = clean(input.value, 500);
  if (!value) throw new Error('DNS value is required.');
  const matches = await callApi('/api/dns/manage/search-records.json', { 'domain-name': domainName, type: input.type, host: host || '@', value, 'no-of-records': 10, 'page-no': 1 }, 'GET');
  if (Number(matches.recsindb || matches.recsonpage || 0) > 0) return { success: true, existing: true };
  const conflictTypes = input.type === 'TXT' ? ['CNAME'] : input.type === 'A' ? ['A', 'AAAA', 'CNAME'] : ['A', 'AAAA', 'CNAME', 'TXT', 'MX'];
  for (const type of conflictTypes) {
    const existing = await callApi('/api/dns/manage/search-records.json', { 'domain-name': domainName, type, host: host || '@', 'no-of-records': 10, 'page-no': 1 }, 'GET');
    if (Number(existing.recsindb || existing.recsonpage || 0) > 0) throw new Error('Existing DNS records conflict with this hosting change. Review them manually; no records were removed.');
  }
  await callApi(`/api/dns/manage/${endpoint}.json`, { 'domain-name': domainName, ...(host ? {host} : {}), value, ttl: 600 }, 'POST');
  return { success: true, existing: false };
}

async function renewRegistration(input) {
  const before = await getRegistration(input);
  if (before.expiresAt !== input.expectedExpiry) throw new Error('Registrar expiry changed. Reconcile before renewing.');
  await callApi('/api/domains/renew.json', { 'order-id': before.orderId, years: input.years, 'exp-date': Math.floor(new Date(before.expiresAt).getTime()/1000), 'invoice-option': 'NoInvoice', 'auto-renew': false }, 'POST');
  const after = await getRegistration(input);
  if (new Date(after.expiresAt) <= new Date(before.expiresAt)) throw new Error('Registrar renewal is pending confirmation.');
  return after;
}

module.exports = {
  getPlatformFundingBalance,
  getRegistration,
  addPlatformDnsRecord,
  renewRegistration,
  checkAvailability,
...
// Platform quotes use OUR cost, not customer selling prices or sub-reseller slabs.
async function getPlatformCostPrice(input) {
 const domainName=normalizeDomain(input.domainName),parts=splitDomain(domainName);
 const years=Number(input.years);if(!Number.isInteger(years)||years<1||years>5)throw new Error('Invalid domain term.');
 const [catalog,details]=await Promise.all([callApi('/api/products/reseller-cost-price.json',{}),callApi('/api/resellers/details.json',{})]);
 const product=resolveProductForTld(catalog,parts.tld).product;
 const action=input.operation==='renew'?'renewdomain':'addnewdomain';
 const amountMinor=normalizeMoneyMinor(product?.[action]?.[String(years)]);
 // The upstream price is in the parent's selling currency, not our display
 // currency (NGN) or accounting currency (GBP). Fail closed if missing.
 const currency=normalizeCurrencyCode(details?.parentsellingcurrencysymbol);
 if(!Number.isSafeInteger(amountMinor)||amountMinor<=0||!['USD','GBP','NGN'].includes(currency))throw new Error('Current domain cost could not be confirmed. Please try again shortly.');
 return {amountMinor,currency,priceSource:'reseller-cost-price',fetchedAt:new Date().toISOString()};
}

async function getPlatformFundingBalance() {
 const [data,details]=await Promise.all([
  callApi('/api/billing/reseller-balance.json',{'reseller-id':authParams()['auth-userid']}),
  callApi('/api/resellers/details.json',{})
 ]);
 // For our own account the debit balance and upstream cost use the parent's
 // selling currency. The response can echo OUR retail/accounting currencies;
 // never use the accounting conversion as spendable registrar funds.
 const currency=normalizeCurrencyCode(details.parentsellingcurrencysymbol);
 const value=data.sellingcurrencybalance;
 if(!['USD','GBP','NGN'].includes(currency)||value===null||value===undefined||String(value).trim()==='')throw new Error('Registrar available balance could not be verified.');
 const amount=Number(value),amountMinor=Math.round(amount*100);
 if(!Number.isFinite(amount)||!Number.isSafeInteger(amountMinor))throw new Error('Registrar available balance could not be verified.');
